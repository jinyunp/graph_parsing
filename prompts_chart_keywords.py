...
SCHEMA_HINT = """스키마 (필드명 고정):
{
  ...
  "x_axis": { "name": null, "unit": null, "is_inferred": false, "scale": "linear|log|category|datetime|unknown", "ticks_examples": [] },
  "y_axis": { "name": null, "unit": null, "is_inferred": false, "scale": "linear|log|category|datetime|unknown", "ticks_examples": [] },
...
def make_user_prompt(keywords_min: int = 10, keywords_max: int = 15) -> str:
    ...
    return (
        ...
        "요구 사항:\n"
        f"- 우선 그래프 구조 정보를 정확히 추출하여 스키마에 채우세요(제목/축/범례/시리즈/눈금 등).\n"
        ...
        "형식 엄수:\n"
        "- key_phrases는 아래 필드에서만 발췌/추론 허용: "
        "  title.text, x_axis.name/unit/scale, y_axis/secondary_y_axis 동일, legend.labels, "
        "  series[].summary/label, 축 눈금 범위 요약.\n\n"
        f"{SCHEMA_HINT}"
    )
